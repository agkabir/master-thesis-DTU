...
def raw_radardata_to_rain(file_path):
    ...
    rain_data = rain_data*100 # multiplying by a factor of 100
    rain_data = np.nan_to_num(rain_data,nan = 255) # set nan values by 255
    rain_data = rain_data.astype('uint16')
    return rain_data
...
def write_to_memmap_rain_data(file_path,mf,idx):
    ...
    mf[idx,:,:]=dbz_data
    zero_per = round((dbz_data==0).sum()*100/(1472*1760),3)
    nan_per = round((dbz_data==255).sum()*100/(1472*1760),3)
